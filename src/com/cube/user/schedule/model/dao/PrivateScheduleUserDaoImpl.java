...
public class PrivateScheduleUserDaoImpl implements PrivateScheduleUserDao {
	private static PrivateScheduleUserDao privateScheduleUserDao;
	static {
		privateScheduleUserDao=new PrivateScheduleUserDaoImpl();
	}
	
	public static PrivateScheduleUserDao getPrivateScheduleUserDao() {
		return privateScheduleUserDao;
	}
	private PrivateScheduleUserDaoImpl(){}
	@Override
	public List<ScheduleDto> list(String id) {
		...
		try {
			...
			sql.append("where schedule_kind='개인' \n");
	...
	public void write(ScheduleDto scheduleDto) {
		...
		try {
			...
			sql.append("values(?, schedule_seq.nextval, ?, ?, ?, ?, '개인') \n");
			pstmt=conn.prepareStatement(sql.toString());
			int idx=0;
	...
	public void modify(ScheduleDto scheduleDto) {
		...
		try {
			...
			sql.append("where employee_id=? and schedult_kind='private' \n");
			pstmt=conn.prepareStatement(sql.toString());
			int idx=1;
			pstmt.setString(idx++, scheduleDto.getSchedule_subject());
			pstmt.setString(idx++, scheduleDto.getSchedule_content());
