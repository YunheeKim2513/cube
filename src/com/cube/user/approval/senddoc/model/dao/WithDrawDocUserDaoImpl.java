...
public class WithDrawDocUserDaoImpl implements WithDrawDocUserDao {

	private static WithDrawDocUserDao withDrawDocUserDao;
	
	static{
		withDrawDocUserDao = new WithDrawDocUserDaoImpl();
	}
	
	WithDrawDocUserDaoImpl(){}
	
	public static WithDrawDocUserDao getWithDrawDocUserDao() {
		return withDrawDocUserDao;
	}
	...
	public List<Doc_info_detailDto> withdrawdoclist(String employee_id) {
		...
		try {
			...
			sql.append("from doc_info_detail where employee_id=? and doc_state='결재완료'");
			...
			while(rs.next()){
				...
				withdrawdoclist.add(doc_info_detailDto);
			}
