        ...
        <!-- /#page-wrapper -->
            ...
            <div class="row">
				<div class="col-lg-12">
                    	...
                    	%>
							<button type="button" id="confbtn" name="confbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalok(<%=orderdocDto.getConfirmlist_ok()%>,'<%=orderdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>', '<%=orderdocDto.getDoc_name() %>')">결재</button>
							<!-- <button type="button" id="deferbtn" name="deferbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalok(<%--=orderdocDto.getConfirmlist_ok()%>,'<%=orderdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>', '<%=orderdocDto.getDoc_name() --%>')">보류</button> -->
							<button type="button" id="returnbtn" name="returnbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalcancel(<%=orderdocDto.getConfirmlist_ok()%>,'<%=orderdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>')">반려</button>
						<% 
                    	} else if("temp".equals(request.getParameter("menu"))){
						%>
							<button type="button" id="reportbtn" name="reportbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalreport('<%=orderdocDto.getDoc_seq()%>');">상신하기</button>
	                        <button type="button" id="updatebtn" name="updatebtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalupdate('<%=orderdocDto.getDoc_seq()%>','<%=orderdocDto.getDoc_name()%>');">수정</button>
	                        <button type="button" id="deletebtn" name="deletebtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvaldelete('<%=orderdocDto.getDoc_seq()%>','orderdoc');">삭제</button>
	                    <%
                    	} else if("report".equals(request.getParameter("menu"))){
        				%>
    	                    <button type="button" id="returnbtn" name="deletebtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalsave('<%=orderdocDto.getDoc_seq()%>');">회수</button>
    	                <%
                        } else if("return".equals(request.getParameter("menu"))){
        				%>
    	                	<button type="button" id="newdocbtn" name="newdocbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:newpage('<%=orderdocDto.getDoc_name()%>');">새로작성</button>
    	                <%
                        } else {
                    	%>		
