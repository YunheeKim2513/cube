        ...
        <!-- /#page-wrapper -->
            ...
            <div class="row">
				<div class="col-lg-12">
                    	...
                    	%>
                    		<button type="button" id="confbtn" name="confbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalok(<%=vacdocDto.getConfirmlist_ok()%>,'<%=vacdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>', '<%=vacdocDto.getDoc_name() %>')">결재</button>
							<!-- <button type="button" id="deferbtn" name="deferbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalok(<%--=vacdocDto.getConfirmlist_ok()%>,'<%=vacdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>', '<%=vacdocDto.getDoc_name() --%>')">보류</button> -->
							<button type="button" id="returnbtn" name="returnbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalcancel(<%=vacdocDto.getConfirmlist_ok()%>,'<%=vacdocDto.getDoc_seq()%>', '<%=employeesDto.getEmployee_id()%>')">반려</button>
						<% 
                    	} else if("temp".equals(request.getParameter("menu"))){
						%>
							<button type="button" id="reportbtn" name="reportbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalreport('<%=vacdocDto.getDoc_seq()%>');">상신하기</button>
	                        <button type="button" id="updatebtn" name="updatebtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalupdate('<%=vacdocDto.getDoc_seq()%>','<%=vacdocDto.getDoc_name()%>');">수정</button>
        				...
        				%>
    	                	<button type="button" id="savebtn" name="savebtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:approvalsave('<%=vacdocDto.getDoc_seq()%>');">회수</button>
    	                <%
                        } else if("return".equals(request.getParameter("menu"))){
        				%>
    	                	<button type="button" id="newdocbtn" name="newdocbtn" class="btn btn-primary" style="margin-bottom:7px" onclick="javascript:newpage('<%=vacdocDto.getDoc_name()%>');">새로작성</button>
    	                <%
                        } else if("okreturn".equals(request.getParameter("menu"))){
        				%>
    	                	<!-- <button type="button" id="updatebtn" name="updatebtn" class="btn btn-primary" style="margin-bottom:7px">수정</button> -->
    	                <%
                        } else {
                    	%>		
